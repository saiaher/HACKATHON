...
.metric-card {
    ...
    border-radius: 12px;
    padding: 18px 20px;
    box-shadow: 0 2px 6px rgba(30, 100, 200, 0.08);
    margin-bottom: 10px;
}
.metric-label {
    font-size: 14px;
...
def show_dashboard():
    ...
    with col1:
        ...
        <div class="metric-card">
            ...
            <div class="metric-sub" style="color:#10B981;">✓ Monitoring</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <div class="metric-sub" style="color:#EF4444;">🔴 Urgent</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value" style="color:#F59E0B;">{active_investigations}</div>
            <div class="metric-sub" style="color:#F59E0B;">⚡ Active</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value" style="color:#10B981;">₹{funds_saved // 10000000}Cr</div>
            <div class="metric-sub" style="color:#5A7CA5;">💰 This Month</div>
        </div>
        """, unsafe_allow_html=True)
...
def show_alerts():
    """Alerts page listing all high risk projects"""
    ...
    for project, risk in alerts:
        with st.expander(f"{risk['color']} {project['name']} - Risk: {risk['total_score']}/100"):
            ...
            st.markdown(f"**Location:** {project['district']}, {project['state']}")
            if risk['cost']['flag']:
                st.markdown(f"❌ {risk['cost']['reason']}")
            if risk['timeline']['flag']:
                st.markdown(f"❌ {risk['timeline']['reason']}")
            if risk['ghost']['flag']:
                st.markdown(f"❌ {risk['ghost']['reason']}")
...
def main():
    ...
    # Title
    st.markdown("""
    <h1 style="color:#1E4E8C;">🛡️ MPLAD AI Fraud Detection System</h1>
    """, unsafe_allow_html=True)
